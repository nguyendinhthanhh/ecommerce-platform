...
const SellerOrders = () => {
      ...
      setOrders([
        {
          ...
          productImage: 'https://lh3.googleusercontent.com/aida-public/AB6AXuC34w-NU0fVQbnvbRDU_PvuP9poz6NJVsbWIMPo0EdDVILJ4KT5nhD0Oo8HXARV4LmupFoO82BeTuRZ05IpCkPdIlU5blVTmXLmTQ4mZCH9w4s0JBo0lOWhdSDahKd0HfDRisBvZCcvkj4OkZgryxhs_hk51KbYynZrdUm6p1xyST_CBERmifPbZSD-dgdp67llHXLhyOuhZaouNJ2aIYLS8mKazkQLnQHBd2nso7Pr1eSA_s03TtjExZgzH3AFjaL76boG3TdW5nA',
        ...
        },
        {
          ...
          productImage: 'https://lh3.googleusercontent.com/aida-public/AB6AXuD2HJb5OT_bdf5zOnR1FhwJN2dLzJy3G6-zQWq6spQXxepXqUaYLRoOUe4nN-LAJWrqWMGpmCrJm2t39JFZ7ovsBn3f6pMt6qqeIVJPcXjxw9-xLA-snj4ZwYcCFm5CTGr2BCOefEX-ONg7XcrF3mda5YYQXCofpKpU_6nF1j3L0OrLfsEpaYLBqmwoIpJXNySNPawY1qY05k5KMduM5CwxlN8kqEbkd_1Hya6dO85R8_ERwZhB-2r_mPIZiG-IGL9AujjVT1qXc4w',
        ...
        },
        {
          ...
          productImage: 'https://lh3.googleusercontent.com/aida-public/AB6AXuAoI7Gdco89BXzJUWqYGc4SgCS3RPgWR2TYFLVugLEuw79BOrqg81hFdNguI9WK9rmpfUuYT9O4gmvopq0rtqu8qWiVccWjsPX70IVsMq5oPUfaOrG8bpqtjrd0zcSbL_FV9XT4fRn59VThIRFAfMCxGwh1mm7DFIJ-Eff5j1ITUAVlunchQ3wMCxLpkTCVSZVdays8cWq9ZyQJMmkUrXpXxg9gIAs3L-AKAWkj3J9GzrAV45iF__hRWArLMoQK1mAAmR0fY2L8ZyQ',
  ...
  const getStatusBadge = (status) => {
    ...
    return (
      <span className={`inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full ${config.bg} ${config.text} text-[10px] font-bold uppercase tracking-tight`}>
        <span className={`size-1.5 rounded-full ${config.text.includes('blue') ? 'bg-blue-500' : config.text.includes('orange') ? 'bg-orange-500' : 'bg-green-500'}`}></span>
        {config.label}
      </span>
    );
  };
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
          <div className="bg-white dark:bg-slate-900 p-6 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm">
            ...
            <div className="flex items-end gap-2">
              <p className="text-2xl font-bold tracking-tight">${stats.totalRevenue.toFixed(2)}</p>
              <span className="text-xs font-bold text-green-500 pb-1">+{stats.revenueGrowth}%</span>
            </div>
          </div>
          ...
          <div className="bg-white dark:bg-slate-900 p-6 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm">
            ...
            <div className="flex items-end gap-2">
              <p className="text-2xl font-bold tracking-tight">{stats.pendingOrders}</p>
              <span className="text-xs font-bold text-orange-500 pb-1">+{stats.newOrders} new</span>
            </div>
          </div>

          <div className="bg-white dark:bg-slate-900 p-6 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm">
            <div className="flex items-center justify-between mb-2">
              <span className="text-sm font-medium text-slate-500">To Ship</span>
              <span className="material-symbols-outlined text-blue-500">local_shipping</span>
            </div>
            <div className="flex items-end gap-2">
              <p className="text-2xl font-bold tracking-tight">{stats.toShip}</p>
              <span className="text-xs font-bold text-slate-500 pb-1">Ready for pickup</span>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden flex flex-col">
          ...
          <div className="flex items-center justify-between border-b border-slate-200 dark:border-slate-800 px-6">
            <div className="flex gap-8">
              ...
              >
                Pending
                <span className="absolute top-3 -right-4 px-1.5 py-0.5 bg-orange-500 text-[10px] text-white rounded-full">5</span>
              </button>
              <button 
          ...
          {selectedOrders.length > 0 && (
            <div className="bg-primary/5 px-6 py-2 flex items-center justify-between border-b border-slate-200 dark:border-slate-800">
              <div className="flex items-center gap-4">
                <span className="text-xs font-bold text-primary uppercase tracking-wider">{selectedOrders.length} Orders Selected</span>
                <div className="h-4 w-[1px] bg-primary/20"></div>
                <button className="text-xs font-bold text-primary hover:underline">Batch Print Labels</button>
                <button className="text-xs font-bold text-primary hover:underline">Mark as Shipped</button>
              </div>
              <button 
                ...
                {loading ? (
                  ...
                  orders.map((order) => (
                    <tr 
                      ...
                      style={{ backgroundColor: order.id % 2 === 0 ? 'rgba(37, 19, 236, 0.02)' : 'transparent' }}
                    >
          ...
          {orders.length > 0 && (
            <div className="px-6 py-4 border-t border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-800/30 flex items-center justify-between">
              <p className="text-xs text-slate-500 font-medium">Showing 1 to {orders.length} of 1,240 orders</p>
              <div className="flex items-center gap-1">
                ...
                <button className="size-8 rounded border border-transparent hover:border-slate-200 dark:hover:border-slate-700 flex items-center justify-center text-xs font-medium">2</button>
                <button className="size-8 rounded border border-transparent hover:border-slate-200 dark:hover:border-slate-700 flex items-center justify-center text-xs font-medium">3</button>
                <span className="px-1 text-slate-400">...</span>
                <button className="size-8 rounded border border-transparent hover:border-slate-200 dark:hover:border-slate-700 flex items-center justify-center text-xs font-medium">42</button>
                <button className="size-8 rounded border border-slate-200 dark:border-slate-700 flex items-center justify-center text-slate-400 hover:bg-white dark:hover:bg-slate-800">
