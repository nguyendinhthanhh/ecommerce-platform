...
const AdminOrders = () => {
  ...
  const fetchOrders = async (page = 0) => {
    try {
      ...
      // 1. Lấy danh sách đơn hàng cho bảng (vẫn giữ phân trang)
      ...
      // Apply client-side filters cho danh sách hiện tại
      let filteredOrders = data.content;
      // ... (giữ nguyên logic filter client-side của bạn nếu cần)
      ...
      setPagination({
        ...
        totalElements: data.totalElements, // Lấy tổng số từ Backend trả về thay vì .length
      });

      // 2. GỌI THỐNG KÊ TỔNG QUÁT (Không phụ thuộc vào trang)
  ...
  const fetchFullStats = async () => {
    try {
      ...
      const year = exportDate.year;
      const month = String(exportDate.month).padStart(2, "0");

      const startDate = `${year}-${month}-01`;

      const lastDay = new Date(year, exportDate.month, 0).getDate();
      const endDate = `${year}-${month}-${lastDay}`;

      // Gọi đồng thời cả 2 API để tối ưu tốc độ
      const [statusReport, revenueData] = await Promise.all([
        reportService.getOrderStatusReport(
          filters.dateFrom || null,
          filters.dateTo || null,
          statusList,
        ),
        // Lấy doanh thu dựa trên tháng/năm đang chọn ở exportDate
        // hoặc logic filter của bạn
      ...
      setStats((prev) => ({
        ...
        total: statusReport.reduce((sum, item) => sum + item.total, 0),
        // CẬP NHẬT DOANH THU Ở ĐÂY
  ...
  const executeExport = async () => {
    const loadingToast = toast.loading(
      `Đang xuất dữ liệu tháng ${exportDate.month}/${exportDate.year}...`,
    );
    try {
      const status = filters.status || null;
      const blobData = await reportService.exportOrders(
        exportDate.year,
        exportDate.month,
        status,
      );
      ...
      link.setAttribute(
        "download",
        `Orders_${exportDate.month}_${exportDate.year}.xlsx`,
      );
      ...
      setShowExportModal(false); // Đóng modal sau khi xong
    } catch (error) {
  ...
  return (
    <AdminLayout>
                  ...
                  {[
                    ...
                    .map((event, index, array) => {
                      ...
                      const getColors = () => {
                        if (!isCurrent)
                          return {
                            bg: "bg-white border-2 border-slate-200 dark:bg-slate-900 dark:border-slate-700",
                            text: "text-slate-400",
                            ring: "",
                          };
                        const colors = {
                          blue: {
                            bg: "bg-blue-600",
                            text: "text-white",
                            ring: "ring-4 ring-blue-50 dark:ring-blue-900/30",
                          },
                          indigo: {
                            bg: "bg-indigo-600",
                            text: "text-white",
                            ring: "ring-4 ring-indigo-50 dark:ring-indigo-900/30",
                          },
                          purple: {
                            bg: "bg-purple-600",
                            text: "text-white",
                            ring: "ring-4 ring-purple-50 dark:ring-purple-900/30",
                          },
                          orange: {
                            bg: "bg-orange-600",
                            text: "text-white",
                            ring: "ring-4 ring-orange-50 dark:ring-orange-900/30",
                          },
                          emerald: {
                            bg: "bg-emerald-600",
                            text: "text-white",
                            ring: "ring-4 ring-emerald-50 dark:ring-emerald-900/30",
                          },
                          green: {
                            bg: "bg-green-600",
                            text: "text-white",
                            ring: "ring-4 ring-green-50 dark:ring-green-900/30",
                          },
                          rose: {
                            bg: "bg-rose-600",
                            text: "text-white",
                            ring: "ring-4 ring-rose-50 dark:ring-rose-900/30",
                          },
                        };
                      ...
                      return (
                        ...
                        >
                          ...
                          <div
                            className={`relative z-10 w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0 transition-all ${style.bg} ${style.ring}`}
                          >
                            <span
                              className={`material-symbols-outlined text-[16px] ${style.text}`}
                            >
                              {event.icon}
                            </span>
                          </div>
                          <div className="flex-1 pt-1.5">
                            <p
                              className={`text-sm font-bold ${isCurrent ? "text-slate-900 dark:text-white" : "text-slate-500 dark:text-slate-400"}`}
                            >
                              {event.title}
                            </p>
                  ...
                  <table className="w-full text-left text-sm">
                    ...
                    <tfoot className="bg-slate-50 dark:bg-slate-800/50 border-t border-slate-200 dark:border-slate-800">
                      <tr className="font-medium text-slate-600 dark:text-slate-400">
                        <td colSpan="2" className="px-4 py-2 pt-4 text-right">
                          Subtotal:
                        </td>
                        <td className="px-4 py-2 pt-4 text-right text-slate-900 dark:text-white">
                      ...
                      <tr className="font-medium text-slate-600 dark:text-slate-400">
                        ...
                        <td className="px-4 py-2 pb-4 text-right text-slate-900 dark:text-white">
      ...
      {/* Modal Chọn Tháng Năm để Export */}
      {showExportModal && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4">
          ...
          <div className="relative bg-white dark:bg-slate-900 w-full max-w-md rounded-2xl shadow-2xl p-6">
            ...
            <div className="grid grid-cols-2 gap-4 mb-6">
              <div>
                ...
                <select
                  ...
                  className="w-full px-4 py-2.5 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg"
                >
            ...
            <div className="flex gap-3">
              <button
                onClick={() => setShowExportModal(false)}
                className="flex-1 px-4 py-2.5 bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 rounded-xl font-bold"
              >
                Hủy
              </button>
              <button
                onClick={executeExport}
                className="flex-1 px-4 py-2.5 bg-primary text-white rounded-xl font-bold shadow-lg shadow-primary/20"
              >
                Xác nhận Xuất
              </button>
            </div>
